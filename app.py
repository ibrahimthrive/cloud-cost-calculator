...
PROVIDER_CLASS = {"AWS": "cc-provider-aws", "Azure": "cc-provider-azure", "GCP": "cc-provider-gcp"}
...
def section_title(text: str, icon: str = ""):
    st.markdown(
        f"""<div class="cc-section-title"><span class="bar"></span>{icon} {text}</div>""",
...
def provider_chip(provider: str):
    st.markdown(
        f"""<div class="cc-provider-chip {PROVIDER_CLASS[provider]}">
                <span class="dot"></span>{provider}
            </div>""",
        unsafe_allow_html=True,
    )


def metric_card(label: str, value: str, col, accent: str = "", delay: float = 0.0):
    accent_class = f"cc-accent-{accent}" if accent else ""
    col.markdown(
        f"""<div class="cc-metric-card {accent_class}" style="animation-delay:{delay}s">
...
def render_suggestions(suggestions):
    ...
    for i, s in enumerate(suggestions):
        ...
        st.markdown(
            f"""<div class="cc-suggestion-card" style="animation-delay:{i * 0.08}s">
...
def winner_card(provider: str, monthly_total: float):
    st.markdown(
        f"""<div class="cc-winner-card {PROVIDER_CLASS[provider]}">
                🏆 <b>{provider}</b> is the cheapest option for this configuration at
                <b>${monthly_total:,.2f}/month</b>.
            </div>""",
        unsafe_allow_html=True,
    )
...
def cost_breakdown_chart(compute, storage, egress):
    fig = go.Figure(data=[go.Pie(
        ...
        hole=0.45,
        marker=dict(colors=["#7C3AED", "#34A853", "#F4B400"]),
    )])
...
def monthly_vs_yearly_chart(monthly, yearly):
    fig = go.Figure(data=[
        go.Bar(name="Cost (USD)", x=["Monthly", "Yearly"], y=[monthly, yearly],
               marker_color=["#7C3AED", "#5B21B6"], text=[f"${monthly:,.0f}", f"${yearly:,.0f}"],
...
def page_calculator():
    ...
    with right:
        with st.container(border=True):
            section_title("Estimated Cost", "💵")
            c1, c2, c3 = st.columns(3, gap="medium")
            metric_card("Compute / mo", f"${breakdown['compute_usd']:,.2f}", c1, accent="", delay=0.0)
            metric_card("Storage / mo", f"${breakdown['storage_usd']:,.2f}", c2, accent="green", delay=0.06)
            metric_card("Egress / mo", f"${breakdown['egress_usd']:,.2f}", c3, accent="amber", delay=0.12)

            c4, c5 = st.columns(2, gap="medium")
            metric_card("Monthly Total (USD)", f"${breakdown['monthly_total_usd']:,.2f}", c4, accent="dark", delay=0.18)
            metric_card("Yearly Total (USD)", f"${breakdown['yearly_total_usd']:,.2f}", c5, accent="dark", delay=0.24)

            if target_currency != "USD":
                c6, c7 = st.columns(2, gap="medium")
                metric_card(f"Monthly Total ({target_currency})", f"{monthly_converted:,.2f}", c6, delay=0.3)
                metric_card(f"Yearly Total ({target_currency})", f"{yearly_converted:,.2f}", c7, delay=0.36)
        ...
        with st.container(border=True):
            section_title("Cost Optimization Suggestions", "💡")
            opt_config = {
...
def page_compare():
    page_banner("Compare Providers", "Pick a comparable instance on each provider and shared storage/egress "
                "assumptions to see a side-by-side monthly cost comparison.", "📊")

    with st.container(border=True):
        section_title("Shared Assumptions", "🔧")
    ...
    winner_card(cheapest["Provider"], cheapest["Monthly Total"])

    with st.container(border=True):
        section_title("Side-by-Side Results", "📈")
        st.dataframe(df.style.format({c: "${:,.2f}" for c in df.columns if c != "Provider"}),
                     use_container_width=True, hide_index=True)

        fig = go.Figure(data=[
            go.Bar(name="Compute", x=df["Provider"], y=df["Compute"], marker_color="#7C3AED"),
...
def page_history():
    page_banner("Saved Estimates", "Browse, chart and manage every estimate you've saved from the Calculator.", "🗂")
    ...
    with st.container(border=True):
        section_title("History Table", "📋")
        st.dataframe(df, use_container_width=True, hide_index=True)

        fig = go.Figure(data=[go.Bar(x=df["Name"], y=df["Monthly (USD)"], marker_color="#7C3AED")])
...
def page_mock_api():
    page_banner("Mock REST API", "Spin up a lightweight Flask API in a background thread, backed by the same "
    ...
    with st.container(border=True):
        ...
        if not st.session_state.mock_api_running:
            ...
            st.markdown(
                """<div class="cc-status-pill"><span class="pulse"></span>
                        Mock API running at http://localhost:8502</div>""",
                unsafe_allow_html=True,
            )
...
def page_about():
    page_banner("About", "What this calculator does, and how to interpret its numbers.", "ℹ️")
    with st.container(border=True):
        st.markdown(
            """
...
st.sidebar.markdown(
    """<div class="cc-sidebar-title">☁️ Cost Calculator</div>
       <div class="cc-sidebar-sub">Multi-cloud cost estimation toolkit</div>""",
    unsafe_allow_html=True,
)
